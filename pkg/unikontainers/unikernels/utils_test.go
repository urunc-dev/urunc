...
func TestSubnetMaskToCIDR(t *testing.T) {
	tests := []struct {
		...
		// Non-contiguous netmasks (issue #963)
...
func FuzzSubnetMaskToCIDR(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, maskStr string) {
		cidr, err := subnetMaskToCIDR(maskStr)
		if err != nil {
			return
		}
		if cidr < 0 || cidr > 32 {
			t.Fatalf("subnetMaskToCIDR(%q) returned out-of-range CIDR %d", maskStr, cidr)
		}

		canonicalMask := net.IP(net.CIDRMask(cidr, 32)).String()
		if canonicalMask != maskStr {
			t.Fatalf("subnetMaskToCIDR(%q) = %d, but that CIDR's canonical mask is %q, not %q: mask is not a contiguous run of 1-bits and should have been rejected", maskStr, cidr, canonicalMask, maskStr)
		}
	})
}
